...
function sendMailForImage(text) {
	...
	fu.each('./screenshot', function(item) {
	...
	//数字不能用 直接用i ，会造成只能发十个，到第是一个 就是第一个图了
	for(let i = 0; i < imgPath.length; i++) {
		htmlImg += `<h2>${imgPath[i]}</h2><br/><img src="cid:img${i+123}" id="img${i+123}" style="width:375px;"><br /><hr /><br />`;
		let temp = {
			filename: imgPath[i], //图片名称
